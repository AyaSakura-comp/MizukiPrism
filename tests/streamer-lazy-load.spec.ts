...
test.describe('streamer lazy load', () => {
  test('1: multi-streamer — no songs loaded on mount', async ({ page }) => {
    ...
    const songCount = await page.getByTestId('song-card').count();
  ...
  test('2: click streamer — songs load', async ({ page }) => {
    ...
    const count = await page.getByTestId('song-card').count();
  ...
  test('3: multi-select — accumulate songs', async ({ page }) => {
    ...
    await expect(page.getByTestId('song-card').first()).toBeVisible({ timeout: 15000 });
    const countAfterFirst = await page.getByTestId('song-card').count();

    // Click second streamer (multi-select)
    await buttons.nth(2).click();
    // Wait for new songs to load
    await page.waitForTimeout(3000);
    const countAfterSecond = await page.getByTestId('song-card').count();
  ...
  test('4: deselect all — shows all loaded songs', async ({ page }) => {
    ...
    await buttons.nth(1).click();
    await expect(page.getByTestId('song-card').first()).toBeVisible({ timeout: 15000 });
    const countBefore = await page.getByTestId('song-card').count();
    ...
    const countAfterAll = await page.getByTestId('song-card').count();
